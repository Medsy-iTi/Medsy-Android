...
class EmailVerificationViewModel @Inject constructor() : ViewModel() {
    ...
    private val _events = Channel<EmailVerificationEvent>(capacity = Channel.BUFFERED)
    val events = _events.receiveAsFlow()

    fun onAction(action: EmailVerificationAction) {
        when (action) {
            EmailVerificationAction.OnNextClick -> sendEvent(EmailVerificationEvent.NavigateNext)
        }
    }
